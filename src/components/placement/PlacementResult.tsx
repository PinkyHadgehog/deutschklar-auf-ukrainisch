  ...
  return (
    <Card className="p-8 md:p-10 rounded-3xl border-0 shadow-elevated" data-component="placement-result">
      <div className="text-center">
        ...
        <div className="text-sm text-muted-foreground">{t("placement.result.yourLevel")}</div>
      ...
      <div className="mt-8 grid sm:grid-cols-2 gap-3">
        ...
        <Card className="p-4 rounded-xl border-0 bg-accent-soft">
          <div className="text-xs uppercase text-accent-foreground font-semibold">{t("placement.result.improve")}</div>
      ...
      <div className="mt-6 rounded-xl bg-primary-soft/60 p-4">
        <div className="text-xs uppercase text-primary font-semibold">{t("placement.result.recommendedPath")}</div>
        <div className="font-semibold mt-1">{data.path}</div>
      </div>

      <div className="mt-3 rounded-xl bg-secondary/60 p-4">
        <div className="text-xs uppercase text-muted-foreground font-semibold">{t("placement.result.recommendedCourse")}</div>
      ...
      <div className="mt-8 flex flex-wrap justify-center gap-3">
        ...
        >
          <Link to={data.courseHref}>{t("placement.result.startWith", { level: data.level })} <ArrowRight className="ml-1.5 h-4 w-4" /></Link>
        </Button>
        <Button asChild variant="outline"><Link to="/courses">{t("placement.result.viewCourse")}</Link></Button>
      </div>
    </Card>
  );
};
