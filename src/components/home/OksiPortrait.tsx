 ...
 */
export const OksiPortrait = ({ src, alt }: { src?: string; alt?: string }) => {
  const { t } = useLang();
  return (
    <div className="relative w-full max-w-sm mx-auto lg:mx-0">
      ...
      <div className="relative aspect-[4/5] w-full overflow-hidden rounded-3xl border border-primary/10 bg-primary-soft/50 shadow-elevated">
        {src ? (
          <img src={src} alt={alt ?? t("home.oksi.portraitAlt")} loading="lazy" className="h-full w-full object-cover" />
        ) : (
          <div className="h-full w-full grid place-items-center bg-gradient-to-br from-primary-soft via-secondary to-accent-soft">
            <div className="text-center px-6">
              ...
              <div className="font-display font-bold text-lg mt-4">{t("home.oksi.portraitName")}</div>
              <p className="text-sm text-muted-foreground mt-1">{t("home.oksi.portraitCaption")}</p>
            </div>
          </div>
        )}
      </div>
