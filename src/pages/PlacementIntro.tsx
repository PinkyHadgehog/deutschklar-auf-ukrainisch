...
const PlacementIntro = () => {
  const { t } = useLang();
  const infos = [
    { icon: Clock, label: t("placement.intro.duration") },
    { icon: Layers, label: t("placement.intro.levelRange") },
    { icon: Gift, label: t("placement.intro.free") },
  ];
  ...
  return (
    <div className="container max-w-3xl py-14 md:py-20">
      <Card className="p-8 md:p-12 rounded-3xl border-0 shadow-elevated text-center">
        ...
        <h1 className="font-display text-3xl md:text-4xl font-extrabold">{t("placement.intro.title")}</h1>
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          ...
          >
            <Link to={PLACEMENT_TEST_ROUTE}>{t("placement.intro.start")} <ArrowRight className="ml-1.5 h-4 w-4" /></Link>
          </Button>
          <Button asChild size="lg" variant="outline" className="h-12 px-7">
            <Link to="/">{t("placement.intro.backHome")}</Link>
          </Button>
        </div>

        <p className="text-xs text-muted-foreground mt-6 max-w-lg mx-auto">
          {t("placement.intro.disclaimer")}
        </p>
      </Card>
    </div>
  );
